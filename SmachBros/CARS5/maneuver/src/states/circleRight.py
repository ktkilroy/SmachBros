...
'''
Class Description:
This state is the CircleRight state. Similar to CircleLeft, we pass in the circleRight_input as the input key containing the data recieved from the Command. We then use this param to pass
...
constructs the ackermann message given a speed,angle,duration, and publisher. The biggest difference is that we take the negative of our angle in radians calculated, as this will send the opposite parameter
to the drive state.
'''
class CircleRight(smach.State):
	def __init__(self):
		smach.State.__init__(self, outcomes = ['command'],
					   input_keys=['circleRight_input']	)
	...
	def execute(self, userdata):
		...
		radius = userdata.circleRight_input
		
		circleRight(radius, myDrivePublisher)
